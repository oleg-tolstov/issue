...
public interface IssueRepo extends CrudRepository<Issue, Long> {
	...
	@Query(value = "select em.id as id_emp, em.name as name_emp, e.id as id_equ, e.name as name_equ, e.serial_number, i.date  from issue i, equipment e, employee em where i.equ_id=e.id and i.id_emp=em.id and emo_id = :employeeId;",
			nativeQuery = true)
